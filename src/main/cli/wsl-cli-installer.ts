  ...
  async getStatus(): Promise<CliInstallStatus> {
    ...
    if (content === null) {
      return this.buildStatus({
        ...
        detail: `Register ${ready.commandPath} to use Veer from WSL.`
      })
    }
    ...
    if (content === 'not_file') {
      return this.buildStatus({
        ...
        currentTarget: null,
        pathConfigured: ready.pathConfigured,
        detail: `${ready.commandPath} exists but is not a Veer launcher script.`
      })
    }
      ...
      return this.buildStatus({
        ...
        detail:
          bridgeContent === null || bridgeManaged
            ...
            : `${ready.bridgePath} exists but is not managed by Veer.`
      })
    }
    ...
    return this.buildStatus({
      ...
      pathConfigured: ready.pathConfigured,
      detail: !managed
        ? `${ready.commandPath} exists but is not managed by Veer.`
        : bridgeConflict
          ? `${ready.bridgePath} exists but is not managed by Orca.`
          : `${ready.commandPath} points to a different Veer launcher.`
    })
  }
  ...
  > {
    ...
    if (!hostStatus.launcherPath) {
      return {
        status: this.unsupported(
          ...
          hostStatus.detail ?? 'The Windows Veer CLI launcher is missing.'
        )
      }
    }
    ...
    if (!interopReady) {
      return {
        status: this.unsupported(
          'launcher_missing',
          'WSL Windows interop is unavailable; Veer cannot launch the Windows CLI from WSL.'
        )
      }
    }

    const pathDirectory = `${home}/.local/bin`
    // Why: WSL follows the canonical public Veer command used by all agent surfaces.
