...
function createMockRuntime(): CoordinatorRuntime & {
  ...
  cliCommand: 'veer' | 'veer-ide' | 'orca' | 'orca-ide'
  ...
  const mock = {
    ...
    cliCommand: 'veer' as 'veer' | 'veer-ide' | 'orca' | 'orca-ide',
...
describe('Coordinator', () => {
  ...
  it('dispatches a ready task to an available terminal', async () => {
    ...
    expect(runtime.sentMessages[0].text).toContain('veer-ide orchestration send')
  })
