...
export type DevInstanceIdentity = AppIdentity & {
  ...
  // creating a new one per branch and re-prompting. Distinct from prod's 'Veer'.
  appName: string
}
...
export function getDevInstanceIdentity(
  ...
  return {
    name: dockTitle,
    // Why: one stable Keychain key ('Veer Dev Safe Storage') for all dev
