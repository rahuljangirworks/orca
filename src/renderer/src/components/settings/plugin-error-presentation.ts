...
export function pluginInstallErrorMessage(cause: unknown): string {
  ...
  if (detail.includes('requires orca')) {
    return translate(
      ...
      'This plugin requires a different Veer version.'
    )
  }
  ...
  if (/(exceeds|too many)/.test(detail)) {
    return translate(
      ...
      "The plugin exceeds Veer's install size or file-count limits."
    )
  }
  if (/(git|repository|fetch|clone|checkout|remote)/.test(detail)) {
    return translate(
      'auto.components.settings.pluginError.installGit',
      'Veer could not fetch the pinned Git revision. Check the URL, #ref, access, and system Git setup.'
    )
  }
  return translate(
