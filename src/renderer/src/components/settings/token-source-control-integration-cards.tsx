            ...
            {status === 'unavailable' ? (
              ...
              <>
                ...
                {translate(
                  ...
                  'only when Veer cannot derive the API base URL from the git remote.'
                )}
              </>
            ) : (
              translate(
                'auto.components.settings.token.source.control.integration.cards.40f678df73',
                'Azure DevOps credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart Veer if environment variables changed.'
              )
            )}
          </p>
            ...
            {status === 'unavailable' ? (
              ...
              <>
                ...
                {translate(
                  ...
                  'only when Veer cannot derive the API URL from the remote.'
                )}
              </>
            ) : (
              translate(
                'auto.components.settings.token.source.control.integration.cards.19fb419c12',
                'Gitea credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart Veer if environment variables changed.'
              )
            )}
          </p>
          <div className="flex items-center gap-2">
