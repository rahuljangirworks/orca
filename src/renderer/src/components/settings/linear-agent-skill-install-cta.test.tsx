...
function discoveredSkill(overrides: Partial<DiscoveredSkill>): DiscoveredSkill {
  return {
    ...
    directoryPath: '/Users/test/.agents/skills/veer-linear',
    skillFilePath: '/Users/test/.agents/skills/veer-linear/SKILL.md',
    installed: true,
    updatedAt: null,
    ...overrides
  }
...
describe('LinearAgentSkillInstallCta', () => {
  ...
  it('shows the install command and explanation when the skill is missing', async () => {
    ...
    expect(rendered.textContent).toContain('veer-linear')
    ...
    expect(rendered.textContent).toContain(
      'npx skills add https://github.com/rahuljangirworks/veer --skill veer-linear --global'
    )
  })
  ...
  it('shows a subtle confirmation and the update command when installed', async () => {
    ...
    mocks.skillState.skills = [discoveredSkill({ name: 'veer-linear' })]
    ...
    expect(rendered.textContent).toContain('npx skills update veer-linear --global')
