...
export function getBrowserLinkRoutingDescription(
  ...
  if (modifierInverts) {
    return translate(
      ...
      "Open http(s) links in Veer's built-in browser — from the terminal, markdown, and the editor."
    )
  }
  return translate(
    'auto.components.settings.BrowserLinkRoutingSetting.description',
    "Open http(s) links in Veer's built-in browser — from the terminal, markdown, and the editor. {{shortcut}} always uses your system browser.",
...
export function getLinkRoutingModifierDescription({
  ...
  return openLinksInApp
    ? translate(
        ...
        'Links open in Veer, so {{chord}}+click sends one to your system browser instead.',
        { chord }
      )
    : translate(
        'auto.components.settings.BrowserLinkRoutingModifierSetting.descriptionOrca',
        "Links open in your system browser. When enabled, {{chord}}+click opens one in Veer's built-in browser instead.",
        { chord }
      )
}
