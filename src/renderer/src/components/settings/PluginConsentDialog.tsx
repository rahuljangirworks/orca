              ...
              <span>
                {plugin.hasWorker
                  ? translate(
                      ...
                      "These permissions limit how the plugin uses Veer's API. Its worker still runs as a normal process on your computer with full access to your files, network, and other processes."
                    )
                  : hasInstructionalContent(plugin)
                    ...
                    : plugin.capabilities.length > 0 || plugin.panels.length > 0
                      ? translate(
                          ...
                          "These permissions limit how the plugin uses Veer's API. This plugin has no background worker."
                        )
                      : translate(
                          'auto.components.settings.PluginConsentDialog.declarativeWarning',
                          "This plugin contributes validated content only. It does not run a background worker or receive access to Veer's API."
                        )}
              </span>
            </div>
