...
export async function runOnboardingFeatureSetup(
  ...
  try {
    ...
    if (!status.supported) {
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Veer CLI registration is not available on this platform.'
      })
    } else if (status.pathConfigured === null) {
      // Why: an unknown registry read cannot safely drive a PATH read-modify-write.
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Veer could not check your Windows user PATH.'
      })
      ...
      if (next.state !== 'installed') {
        warnings.push({
          featureId: 'cli',
          message: next.detail ?? 'Veer CLI registration needs attention.'
        })
