        ...
        {configIdentityAlias && form.identityFile.trim() === '' ? (
          <p className="text-xs text-muted-foreground">
            {translate(
              ...
              'Left empty on purpose: Veer uses every key ~/.ssh/config resolves for {{value0}}. Type a path to use just that key.',
    ...
    >
      <div className="space-y-1.5">
        <Label htmlFor="add-server-name">
          {translate('auto.components.sidebar.AddRemoteHostDialog.serverName', 'Name in Veer')}
        </Label>
        <Input
          id="add-server-name"
          value={name}
      ...
      <div className="space-y-1.5">
        ...
        <p id="add-server-pairing-code-help" className="text-xs text-muted-foreground">
          {translate(
            ...
            'Create this under Settings → Remote Veer Servers → Share this host on the other computer.'
          )}
        </p>
        {inputError ? (
