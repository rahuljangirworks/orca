...
function copiedNote(packageFileName: string): string {
  return translate(
    ...
    'Command copied. Run it in a system terminal to install {{value0}}, then quit and reopen Veer.',
    {
...
export function LinuxPackageInstallRecoveryCard({
  ...
  const SUMMARY = translate(
    ...
    'Veer downloaded the update but could not install the system package automatically.'
  )
  const EXPLAINER = translate(
    'auto.components.LinuxPackageInstallRecoveryCard.82c6dbea00',
    'Copy the command and run it in a system terminal on the computer where Veer is installed. After it finishes, quit and reopen Veer to run the new version.'
  )
  ...
  const TRUST_NOTE = translate(
    ...
    'Veer checks the downloaded file against the release metadata at the moment it builds this command. The system package itself is not signature-checked, and Veer cannot vouch for the file after that point.'
  )
