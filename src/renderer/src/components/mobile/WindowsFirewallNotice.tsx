        ...
        if (
          ...
          toast.success(
            translate(
              ...
              'Windows Firewall now allows Veer Mobile on private networks'
            )
          )
          return
        }
              ...
              {networkIsPublic
                ...
                : blockingRuleDetected
                  ? translate(
                      ...
                      'Windows may be blocking Veer Mobile'
                    )
                  : translate(
            ...
            <p className="text-xs text-muted-foreground">
              {networkIsPublic
                ? translate(
                    ...
                    'Change this trusted Wi-Fi network to Private before allowing Veer Mobile connections.'
                  )
                : blockingRuleDetected
                  ? translate(
                      'auto.components.mobile.WindowsFirewallNotice.blocked-description',
                      'An existing inbound Block rule can override the pairing exception. Repair removes conflicting TCP rules for this Veer app, then allows port {{port}} on Private networks.',
                      { port: firewallStatus.port }
                    )
                  : translate(
                      'auto.components.mobile.WindowsFirewallNotice.missing-description',
                      'Windows may block the pairing server. Add a rule for this Veer app and TCP port {{port}} on Private networks.',
                      { port: firewallStatus.port }
                    )}
            </p>
            {usingRelay ? (
              <p className="text-xs text-muted-foreground">
                {translate(
                  ...
                  'Pairing still works over Veer Relay — allowing this only adds the faster local connection.'
                )}
              </p>
            ) : null}
          </div>
