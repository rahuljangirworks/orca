...
export function SkillInstallDialog({
  ...
  const resolveLink = useCallback(async (value: string): Promise<void> => {
    ...
    if (!shareId) {
      setError(
        translate('auto.components.skills.install.enterShareLink', 'Enter a Veer skill share link.')
      )
      return
    }
    setBusy(true)
    ...
    try {
        ...
        setError(
          operation.status === 'reconnect-required'
            ? translate(
                ...
                'Reconnect your Veer account before installing.'
              )
            : operation.message
        )
        return
      ...
      setError(
        translate(
          ...
          'Installation failed before Veer could verify the requested version.'
        )
      )
    } finally {
