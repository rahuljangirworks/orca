...
function humanizeUnreattachableSession(error: string): string {
  const explanation = translate(
    ...
    "Veer couldn't reattach to this pane's terminal session on the host. Open a new terminal to continue."
  )
  ...
  if (humanized.includes(PANE_OWNER_UNVERIFIED_MARKER)) {
    humanized = humanized.replace(
      ...
      translate(
        ...
        "Veer couldn't confirm whether this terminal's previous session is still running, so it left the session untouched. Reopen this pane to retry."
      )
    )
  }
