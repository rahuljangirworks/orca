    ...
    return subscribeToMacosTccPromptNotice(window.api?.macosTccPrompts, (_, acknowledge) => {
      toast.warning(
        translate(
          ...
          'Seeing “Veer would like to access…” prompts?'
        ),
        {
          description: translate(
            'auto.hooks.useMacosTccPromptNotice.description',
            'Permission messages from macOS may appear when an agent or terminal tool running in Veer attempts to access protected files. Grant Full Disk Access in Settings to reduce these prompts.'
          ),
          duration: Infinity,
