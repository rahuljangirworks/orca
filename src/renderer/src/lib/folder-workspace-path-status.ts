...
export function getFolderWorkspacePathStatusDescription(
  ...
  if (!isHandledPathStatusReason(status.reason)) {
    return translate(
      ...
      'Veer cannot use {{path}}, and this version does not recognize the reason. Update Veer to see the details.',
      { path: status.path }
    )
  }
  switch (status.reason) {
    case 'missing':
      return translate(
        ...
        'Veer cannot find {{path}}. Remove and re-import this folder workspace.',
        { path: status.path }
      )
    case 'not-directory':
      return translate(
    ...
    case 'ambiguous-connection':
      return translate(
        ...
        'Veer cannot tell which SSH connection owns this folder scope.'
      )
    case undefined:
    case 'unavailable':
      return translate(
        'auto.lib.folderWorkspacePathStatus.description.unavailable',
        'Veer cannot verify this folder right now. Check the runtime or SSH connection and try again.'
      )
  }
}
  ...
  if (message.startsWith('folder_workspace_path_missing:')) {
    return {
      ...
      description: translate(
        ...
        'Veer cannot find {{path}}. Remove and re-import the folder.',
        { path }
      )
    }
  }
