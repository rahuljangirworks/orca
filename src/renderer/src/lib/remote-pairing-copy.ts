  ...
  switch (kind) {
    case 'invalid-input':
      return translate(
        ...
        'Enter a Veer access link or bare pairing code.'
      )
    case 'mobile-only':
      return translate(
        'auto.lib.remotePairingCopy.mobileOnly',
        'This link grants mobile-only access. Generate a link for another Veer client.'
      )
  ...
  switch (kind) {
    case 'host-identity-mismatch':
      return translate(
        ...
        'Veer reached {{endpoint}}, but that host does not match this link. Generate a new link on the other host.',
    ...
    case 'protocol-incompatible':
      return translate(
        ...
        'Update Veer on this device and the other host, then try again.'
      )
    ...
    case 'environment-save-failed':
      return translate(
        ...
        'The host was verified, but Veer could not save it. Check the name and local settings storage, then try again.'
      )
    case 'host-unreachable':
      return translate(
        'auto.components.settings.RuntimeHostAccessForm.unavailableHelp',
        'Make sure Veer is running on the other host and that the network or SSH tunnel can reach {{endpoint}}.',
        { endpoint: endpoint ?? 'Orca' }
      )
  }
}
