...
describe('getLinearAgentSkillUpdateTarget', () => {
  it('targets the canonical skill for missing, canonical, and dual-name installs', () => {
    expect(getLinearAgentSkillUpdateTarget([], false)).toEqual({
      skillName: 'veer-linear',
      command: VEER_LINEAR_SKILL_UPDATE_COMMAND
    })
    expect(getLinearAgentSkillUpdateTarget([skill('veer-linear')], true)).toEqual({
      skillName: 'veer-linear',
      command: VEER_LINEAR_SKILL_UPDATE_COMMAND
    })
    expect(
      getLinearAgentSkillUpdateTarget([skill('veer-linear'), skill('linear-tickets')], true)
    ).toEqual({
      skillName: 'veer-linear',
      command: VEER_LINEAR_SKILL_UPDATE_COMMAND
    })
