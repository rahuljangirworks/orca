      ...
      [
        {
          ...
          summary: 'Permissions',
          usage: 'veer emulator permissions <op> <package> [permission]',
  ...
  const specs: CommandSpec[] = [
    {
      ...
      usage: 'veer worktree rm --worktree <selector>',
      allowedFlags: ['worktree', 'force']
    },
    {
      path: ['repo', 'show'],
      summary: 'Show a repo',
      usage: 'veer repo show --repo <selector>',
      allowedFlags: ['repo'],
      positionalArgs: ['repo']
    }
  ]
...
describe('validateCommandAndFlags', () => {
  ...
  it('attaches did-you-mean suggestions to unknown-command errors', () => {
    ...
    try {
      ...
      expect(data?.nextSteps[0]).toContain('veer worktree rm')
    }
  })
})
