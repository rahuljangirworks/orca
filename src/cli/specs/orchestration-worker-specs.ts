...
export const ORCHESTRATION_WORKER_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    notes: [
      ...
      '--agent accepts any installed agent id: claude, claude-agent-teams, openclaude, codex, opencode, pi, gemini, antigravity, kilo, cursor, hermes, omp, grok, droid, aider, goose, amp, and others. Run `veer agent list` or check Veer Settings > Agents to see what is installed.',
      ...
      '--on selects only the worker server; the Run and this command remain on the current Veer server.',
  ...
  },
  {
    ...
    usage: 'veer orchestration worker-show --dispatch <dispatch_id> [--json]',
    ...
    notes: [
      ...
      'observation.agentWait names a worker parked on a prompt only a human can answer, with the evidence that proved it (hook, prompt-text, or title). Null means Veer looked and found no wait. An absent field means it never looked — an older host, an unverifiable worker identity, an unreadable pane, or an agent probe that did not answer in time — and never means the worker is not waiting. A waiting worker is healthy, not failed.'
    ]
  },
  {
    ...
    usage:
      'veer orchestration worker-read --dispatch <dispatch_id> [--source <auto|transcript|terminal>] [--cursor <cursor>] [--limit <n>] [--json]',
    ...
    notes: [
      ...
      'A returned cursor is pinned to the exact source; start a fresh read if Veer reports source_changed.'
    ]
  },
  {
    ...
    usage:
      'veer orchestration worker-stop --dispatch <dispatch_id> [--retry-request <id>] [--json]',
  ...
  },
  {
    ...
    usage:
      'veer orchestration worker-abandon --dispatch <dispatch_id> [--retry-request <id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'dispatch', 'retry-request'],
  ...
  },
  {
    ...
    usage:
      'veer orchestration worker-release --dispatch <dispatch_id> [--retry-request <id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'dispatch', 'retry-request'],
    notes: [
  ...
  },
  {
    ...
    usage:
      'veer orchestration worker-retain --dispatch <dispatch_id> [--retry-request <id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'dispatch', 'retry-request'],
    notes: [
  ...
  },
  {
    ...
    usage:
      'veer orchestration worker-list [--run <run_id>] [--terminal-state <active|reclaimable|retained|release_pending|release_unknown|released>] [--json]',
