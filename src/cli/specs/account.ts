...
export const ACCOUNT_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['account', 'add'],
    summary: 'Add a managed Claude or Codex account by signing in on this Veer host',
    usage: 'veer account add [--agent claude|codex] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'agent'],
    notes: [
      'Runs the agent login (`claude login` / `codex login`) in this terminal, then registers the account with the local Veer runtime.',
      ...
      '--agent defaults to claude. Requires the Veer runtime to be running on this machine.'
    ],
    examples: ['veer account add', 'veer account add --agent codex']
  },
  {
    path: ['account', 'list'],
    summary: 'List managed Claude and Codex accounts on this Veer host',
    usage: 'veer account list [--json]',
