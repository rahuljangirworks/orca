...
describe('veer computer observation CLI handlers', () => {
  beforeEach(() => {
    vi.restoreAllMocks()
    callMock.mockReset()
  ...
  it('prints targeted permission setup command help', async () => {
    ...
    expect(output).toContain(
      'veer computer permissions [--id <accessibility|screenshots>] [--json]'
    )
  ...
  it('opens computer permission setup without resolving a worktree', async () => {
    ...
    const output = vi.mocked(console.log).mock.calls[0][0]
    expect(output).toContain('Opened Veer Computer Use permission setup')
