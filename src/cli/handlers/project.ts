...
// Why: an Veer server that predates project host setup answers `method_not_found`, which reads
// as an Veer bug rather than a version gap — and since --host runtime:<id> now routes these
  ...
  try {
    ...
    if (error instanceof RuntimeClientError && error.code === 'method_not_found') {
      throw new RuntimeClientError(
        ...
        'This Veer server does not support project host setup yet. Update Veer on the server and try again.'
      )
    }
    throw error
  }
