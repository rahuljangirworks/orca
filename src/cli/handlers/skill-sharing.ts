...
function rejectForwardedSkillFilesystem(ctx: HandlerContext, command: string): void {
  ...
  throw new RuntimeClientError(
    'invalid_environment',
    `veer skills ${command} must run on the machine whose installed skills you want to use. Run the command from a Veer terminal on that machine.`
  )
}
...
function requireCloudOperation<T>(operation: SkillCloudOperation<T>): T {
  ...
  if (operation.status === 'reconnect-required') {
    throw new RuntimeClientError('authentication_required', 'Sign in to Veer and try again.')
  }
...
async function callShare(
  ...
  try {
    ...
    if (error instanceof RuntimeRpcFailureError && error.code === 'method_not_found') {
      throw new RuntimeClientError(
        'update_required',
        'The connected Veer runtime does not support agent skill sharing yet. Update Veer on that machine and try again.'
      )
    }
    throw error
  }
...
export const SKILL_SHARING_HANDLERS: Record<string, CommandHandler> = {
  ...
  'skills share': async (ctx) => {
    ...
    if (skillSelectors.length === 0) {
      throw new RuntimeClientError(
        'invalid_argument',
        'Select at least one installed skill with --skill. Run `veer skills installed` to list them.'
      )
    }
