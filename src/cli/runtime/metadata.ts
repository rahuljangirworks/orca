  ...
  try {
    ...
    if (!metadata || !findTransport(metadata, 'unix', 'named-pipe') || !metadata.authToken) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        `Veer runtime metadata is incomplete at ${metadataPath}`
      )
    }
    return metadata
  } catch (error) {
    ...
    throw new RuntimeClientError(
      'runtime_unavailable',
      `Could not read Veer runtime metadata at ${metadataPath}. Start the Veer app first.`
    )
  }
}
...
export function getDefaultUserDataPath(
  ...
  // Why: in dev mode (and for parallel Veer instances), the Electron app writes
  ...
  if (platform === 'win32') {
    ...
    if (!appData) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        'APPDATA is not set, so the Veer runtime metadata path cannot be resolved.'
      )
    }
