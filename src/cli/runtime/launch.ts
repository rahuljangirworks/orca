...
export function launchOrcaApp(): void {
  ...
  throw new RuntimeClientError(
    'runtime_open_failed',
    'Could not determine how to launch Veer. Start Veer manually and try again.'
  )
}
    ...
    const onClose = (code: number | null, signal: NodeJS.Signals | null): void => {
      ...
      finish(
        new RuntimeClientError(
          ...
          typeof code === 'number'
            ? `Veer serve exited before printing valid recipe JSON with code ${code}.`
            : `Veer serve exited before printing valid recipe JSON via ${signal}.`
        )
      )
    }
...
function resolveAppRoot(): string {
  ...
  // process.cwd(). Pin it to the app root so `veer serve` behaves the same
...
function resolveForegroundOrcaExecutable(): string {
  ...
  throw new RuntimeClientError(
    'runtime_serve_failed',
    'Could not determine how to start Veer server. Set ORCA_APP_EXECUTABLE to the Veer executable.'
  )
}
