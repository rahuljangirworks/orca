...
// Why: a paired Veer server and an SSH target are different machines reached different ways, but
...
export function crossKindNextSteps(
  ...
  if (requested !== 'ssh' && ssh) {
    steps.push(
      `"${name}" is an SSH target on this Veer host, not a paired server. Use --host ssh:${ssh.id}.`
    )
  }
  if (requested !== 'environment' && environment) {
    steps.push(
      `"${name}" is a paired Veer server, not an SSH target. Use --environment ${environment.name}.`
    )
  }
  return steps
}
...
export async function resolveSshHostTargetId(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    `Unknown SSH target in --host ssh:${targetId}: this Veer host has no SSH target named or with id ${targetId}.`,
    {
      ...
      nextSteps: [
        ...
        ...(targets.length > 0
          ...
          : ['This Veer host has no SSH targets registered.'])
      ]
    }
  )
}
