...
export async function resolveCurrentWorktreeSelector(
  ...
  if (!enclosingWorktree) {
    throw new RuntimeClientError(
      'selector_not_found',
      `No Veer-managed worktree contains the current directory: ${currentPath}`
    )
  }
...
// like `veer terminal send --text "hello" --enter` Just Work.
...
export type EmulatorCliTarget = {
  worktree?: string
  device?: string
  emulator?: string // Veer id from list
}
