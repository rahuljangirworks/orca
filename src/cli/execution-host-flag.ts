...
export type HostFlagRoutingSelection = {
  // Why: SSH targets live in the running Veer host, not on disk, so enumerating them needs a
...
export async function resolveHostFlagEnvironmentId(
  ...
  if (!environment) {
    ...
    throw new RuntimeClientError(
      'invalid_argument',
      `Unknown Veer server in --host ${host.id}: no paired Veer server is named or has id ${host.environmentId}.`,
      {
        ...
        nextSteps: [
          ...
          'Run `veer environment list` to see paired Veer servers.',
  ...
  if (selection.pairingCode) {
    throw new RuntimeClientError(
      'invalid_argument',
      `--host ${host.id} already selects a paired Veer server; use either --host runtime:<id> or --pairing-code, not both.`
    )
  }
  if (selection.environmentSelector) {
    ...
    if (selected.id !== environment.id) {
      throw new RuntimeClientError(
        'invalid_argument',
        `--host ${host.id} and ${selection.environmentSelector.label} ${selection.environmentSelector.value} name different Veer servers.`
      )
    }
  }
...
function assertEnvironmentNameUnambiguous(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    `Ambiguous Veer server in ${flag}: ${ambiguous.length} paired servers are named ${name}. Use the environment id.`,
    {
...
export async function assertEnvironmentSelectorResolvable(
  ...
  throw new RuntimeClientError(
    'invalid_argument',
    `Unknown Veer server in --environment ${selector}: no paired Veer server is named or has id ${selector}.`,
    {
      knownEnvironments: environments,
      knownSshTargets: sshTargets,
      nextSteps: [
        ...crossKindNextSteps(selector, { environments, sshTargets }, 'environment'),
        'Run `veer host list` to see every machine you can target and the flag for each.'
      ]
    }
  )
}
