  ...
  useEffect(() => {
    ...
    const hydrateTaskState = async (): Promise<void> => {
      ...
      if (!status.capabilities?.includes(MOBILE_TASKS_CAPABILITY)) {
        ...
        setMergeMethodProjectRow(null)
        resetWorkspaceCreateState()
        setError('Update Veer desktop to use Tasks on mobile.')
        setTaskStateHydrated(false)
        return
      }
    ...
    async (project: GitHubProjectRef, options: { viewNumber?: number } = {}): Promise<void> => {
      ...
      try {
        ...
        if (explicitView && explicitView.layout !== 'TABLE_LAYOUT') {
          throw new Error("Veer doesn't support this GitHub Project layout yet.")
        }
  ...
  const createWorkspaceFromProjectRow = useCallback(
    async (row: GitHubProjectRow): Promise<void> => {
      ...
      if (!kind || !row.content.number || !row.content.url) {
        setError('Add the project item repository to Veer before creating a workspace.')
        return
      }
      if (!repo) {
        const slug = splitRepositorySlug(row.content.repository)
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {!tasksSupported ? (
        tasksUnsupported ? (
          <View style={styles.centered}>
            <Text style={styles.emptyText}>Update Veer desktop</Text>
      ...
      <PickerModal
        ...
        onSelect={(viewId) => {
          ...
          if (view && view.layout !== 'TABLE_LAYOUT') {
            setGithubProjectError("Veer doesn't support this GitHub Project layout yet.")
            return
          }
      ...
      >
        {projectRepoNotInOrca ? (
          <View>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>Repository not in Veer</Text>
              <Text style={styles.sheetSubtitle}>
                {projectRepoNotInOrca.owner}/{projectRepoNotInOrca.repo} is not added to Veer. Add
            ...
            <View style={styles.actionGroup}>
              ...
              projectRowItem.content.state !== 'MERGED' ? (
                <>
                  ...
                  {!projectRowHostedRepo ? (
                    <Text style={styles.emptyInlineText}>
                      Merge requires this repository in Veer.
                    </Text>
                  ) : null}
                </>
              ) : null}
