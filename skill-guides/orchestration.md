...
# Veer Inter-Agent Orchestration

Orchestration is Veer's structured coordination layer for agent messages, task ownership, dispatch state, and worker completion tracking.
...
If a task says to use Veer orchestration, the coordinator must create or bind a Run, create the Task with `veer orchestration task-create`, then attach the worker with either the preferred `veer orchestration worker-start` composition or the low-level `veer orchestration dispatch --inject` path.

Do not substitute non-Veer subagent tools, generic agent-spawn APIs, or chat-only parallel worker features. Those may create useful workers, but they do not create Veer task/dispatch provenance, injected lifecycle preambles, `worker_done` authority, or decision gates.

Before claiming a worker was orchestrated, verify the task/dispatch exists:

```bash
veer orchestration task-list --json
veer orchestration dispatch-show --task <task_id> --json
```

If the work was accidentally run outside Veer orchestration, say so plainly. To repair provenance, rerun or revalidate the needed work through a fresh Veer terminal plus injected dispatch; do not retroactively describe the external worker as orchestrated.
...
- `veer status --json` should show a running runtime.
- `veer` must be on PATH (`veer` on Linux).
- The orchestration experimental feature must be enabled in Settings > Experimental.
- `veer orchestration` commands are RPC calls to the running Veer runtime.

## Contract Migration

Veer adopts a live pre-update orchestration assignment into an ordinary Run. Adoption preserves the existing agent process, PTY/session, terminal handle, tab/leaf/pane, worktree or folder workspace, Task, and Dispatch; it never restarts or replaces the worker. The retired scheduler is not revived, and a newly created attempt uses the current grammar.
...
```bash
veer orchestration run-use --id <adopted_run_id> --takeover-legacy --json
veer orchestration check --run <adopted_run_id> --json
```
...
New orchestration messages and tasks belong to one explicitly bound Run. A Run is only a durable namespace and coordinator inbox; it never schedules or places workers. Lifecycle authority comes from the active Dispatch, and terminal handles remain routing metadata rather than durable identity. Send `worker_done` and `heartbeat` from the worker's own terminal; Veer routes them to that Dispatch's Run.
...
- Do not use `veer orchestration dispatch --inject` for full handoffs. It injects a coordinator preamble that tells the worker to send `worker_done`, heartbeat, and `ask` messages, then end its turn under the original terminal's dispatch lifecycle.
- Do not run `veer orchestration task-create`, `veer orchestration dispatch --inject`, or `veer orchestration check --wait` for full handoffs. Do not peek at terminal output after prompt delivery to monitor progress.
...
```bash
veer orchestration send --subject <text> [--to <run:id|dispatch:id|legacy_handle>] [--from <handle>] [--body <text>] [--type <type>] [--priority <level>] [--thread-id <id>] [--payload <json>] [--json]
veer orchestration check [--terminal <handle>] [--ack <delivery_id>] [--peek|--all] [--types <type,...>] [--format] [--wait] [--timeout-ms <n>] [--json]
veer orchestration reply --id <msg_id> --body <text> [--from <handle>] [--json]
veer orchestration ask (--question <text>|--resume <msg_id>) [--options <csv>] [--timeout-ms <n>] [--from <handle>] [--json]
veer orchestration inbox [--limit <n>] [--json]
```

Rules:

- Omit `--from` unless impersonating another terminal; Veer auto-resolves it from the current terminal.
...
- Use `dispatch:<id>` for coordinator guidance to one supervised worker. Veer routes that stable address locally or through the connected-server relay; do not substitute a remote terminal handle.
- Terminal handles remain appropriate for low-level pre-Dispatch messaging. Prefer `agentTerminalHandle` from the create response, fall back to `startupTerminal.handle` for older runtimes, then re-resolve with `veer terminal list --worktree ... --json` if missing or stale. Continue with the replacement handle only; never dual-send to old and new handles.
...
- `veer orchestration check --peek --format --json` returns locally formatted unread mail without consuming it; it never writes to terminal input or remotely wakes another terminal. Use `orchestration dispatch --inject` to deliver a tracked task, or `terminal send` when an existing agent needs a free-form prompt.
- While supervising workers manually, use `check --wait --types worker_done,escalation,question --timeout-ms <n>` instead of sleep/poll loops. Process the whole Delivery, reply to `question` messages with `veer orchestration reply --id <msg_id> --body <answer> --json`, then acknowledge and keep waiting.
...
- `heartbeat` is also Dispatch-scoped. Include both IDs and omit `--to` so Veer uses the owning Run; use `status` for broad progress updates.
...
```bash
veer orchestration run-create --objective <text> --json
veer orchestration task-create --spec <text> [--deps <json_array>] [--parent <task_id>] [--json]
veer orchestration task-list [--status <status>] [--ready] [--brief] [--json]
veer orchestration task-update --id <task_id> --status <status> [--result <json>] [--json]
veer orchestration dispatch --task <task_id> --to <handle> [--from <handle>] [--inject] [--json]
veer orchestration dispatch-show --task <task_id> [--json]
```
...
- If the target is a bare shell, omit `--inject`, dispatch for tracking if needed, then send the prompt manually with `veer terminal send --terminal <handle> --text <prompt> --enter --json`.
...
- **It is a guardrail, not a security boundary.** A caller that declares another terminal's
  ...
  example) can be counted as that terminal instead. Veer does not treat workers as hostile.
...
Use `worker-start` for the normal supervised path. It composes the existing worktree, terminal, readiness, and dispatch primitives while returning exact created/reused effects. Agents still choose placement and concurrency; Veer does not schedule workers or infer conflicts.

Create the Run and every independent Task first, then start all independent workers before waiting:

```bash
veer orchestration run-create --objective "<objective>" --json
veer orchestration task-create --spec "<worker A task>" --json
...
```bash
veer orchestration worker-start --task <task_id> --worktree current --agent claude --model opus --effort high --json
```

`--effort` requires `--model`, and neither option can combine with `--terminal`. A connected worker server must advertise launch-preference support before Veer forwards either option.

For a new worktree, setup runs by default and agent-first creation reuses the returned startup agent terminal:

```bash
veer orchestration worker-start --task <task_id> --worktree new-child --name <name> --agent codex --setup run --json
# Independent/top-level:
veer orchestration worker-start --task <task_id> --worktree new-top-level --name <name> --agent codex --setup run --json
```
...
To run the worker on another connected Veer server, add `--on <saved-environment>`. The Run and Tasks remain authoritative on the current server; later commands route by Dispatch ID, so never repeat `--on`:

```bash
# Mac Run home -> Windows worker (the reverse is identical from a Windows Run home)
veer orchestration worker-start --task <task_id> --on windows --worktree new-top-level --repo <exact_remote_repo_selector> --name <name> --agent codex --setup run --json
veer orchestration worker-show --dispatch <dispatch_id> --json
veer orchestration worker-read --dispatch <dispatch_id> --limit 50 --json
veer orchestration send --to dispatch:<dispatch_id> --subject "Follow-up" --body "<attempt-specific guidance>" --json
```
...
`orchestration check` receives it even when the Dispatch is on another connected Veer server.

`worker-read` defaults to `--source auto`: Veer returns the exact hook-reported Codex, Claude, OpenClaude, or Grok transcript when it can prove the worker session, otherwise it returns bounded terminal output with `source: "terminal"` and a typed `fallbackReason`. Continue with the returned top-level `cursor`; it stays pinned to that exact source. If Veer reports `source_changed`, start a fresh read without the old cursor. Never supply or guess a provider session ID or transcript path.

Wait until every expected Dispatch settles, not for a fixed number of batches:

```bash
veer orchestration check --wait --types worker_done,escalation,question --timeout-ms 900000 --json
...
veer orchestration check --ack <delivery_id> --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```

After processing each accepted `worker_done`, choose the terminal's next owner before you acknowledge the Delivery or wait again. If the same exact agent has an immediate follow-up Task, read the `worker.agent_terminal_handle` field of `worker-show --dispatch <dispatch_id> --json`, then run `veer orchestration worker-start --task <next_task_id> --terminal <handle> --json` so Veer transfers cleanup ownership to the new Dispatch. Otherwise run `veer orchestration worker-release --dispatch <dispatch_id> --json`.

Run `worker-release` after both succeeded and failed `worker_done` reports unless the user explicitly asked to keep that worker live. Release is post-completion cleanup, not cancellation: Veer first preserves inspectable output, then closes only the exact agent terminal owned by that settled Dispatch. Reused or pre-existing terminals, setup terminals, coordinators, active workers, user-taken-over terminals, and identities Veer cannot prove are retained. If the user explicitly asks to keep the live terminal for debugging, record that exception with `veer orchestration worker-retain --dispatch <dispatch_id> --json` instead of silently skipping cleanup. When the user is finished, the same Dispatch can be passed to `worker-release`, which clears the requested retention and releases the terminal.
...
Workers report exactly once using the IDs and capability injected by Veer; they do not supply Run/server/terminal identity:

```bash
veer orchestration send --type worker_done --subject "<status>" --body "<what changed, findings, and what remains>" --task-id <task_id> --dispatch-id <dispatch_id> --outcome succeeded --files-modified "path/a,path/b" --json
...
```bash
veer orchestration ask --question "<question>" --options "yes,no" --timeout-ms 600000 --json
veer orchestration ask --resume <message_id> --timeout-ms 600000 --json
# Coordinator:
veer orchestration reply --id <message_id> --body "<answer>" --json
```
...
```bash
veer orchestration gate-create --task <task_id> --question <text> [--options <json_array>] [--json]
veer orchestration gate-resolve --id <gate_id> --resolution <text> [--json]
veer orchestration gate-list [--task <task_id>] [--status <status>] [--json]
```
...
Recovery only: `veer orchestration reset --tasks|--messages|--all --json` clears the selected local orchestration database state. Do not run it during active coordination unless explicitly abandoning that state.
...
```bash
veer worktree create --name <task-name> --no-parent --agent codex --prompt "<task brief>" --setup run --json
```

Before creating a new worktree from an active feature branch, decide and state whether the desired Veer lineage is child or top-level. Use child worktree lineage only when the new work is conceptually stacked under or dependent on the active worktree. For independent repo-wide fixes, standalone feature work, or unrelated follow-up tasks, create a top-level worktree with `--no-parent`.
...
`veer worktree create --agent codex --prompt ...` launches the known Codex agent but does not accept Codex-specific `--model` or `-c model_reasoning_effort=...` arguments. When the user asks for a specific Codex model or effort, create the independent worktree first, launch Codex with the requested command in that worktree, wait only for TUI readiness if prompt delivery would otherwise race startup, send the prompt, and stop.
...
Use the exact full `<repo-id>::<path>` worktree id returned by `veer worktree create --json`; a bare repo id cannot target the new worktree.

```bash
veer worktree create --name <task-name> --no-parent --setup run --json
veer terminal create --worktree id:<newFullWorktreeId> --title <task-name> --command 'codex --model gpt-5.5 -c model_reasoning_effort="xhigh"' --json
...
`--no-parent` only controls Veer lineage; it does not choose the Git base. If the work should start from the repo default base, omit `--base-branch` so Veer uses that default, or explicitly pass the repo default base (`origin/main`, `origin/master`, or the `veer repo show --repo <selector> --json` value); never base it on the current feature branch unless the user explicitly asks for stacked work or "branch from current". Put current-branch context in the prompt instead.
...
```bash
veer terminal create --worktree active --title <task-name> --command "codex" --json
veer terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
...
When a new worktree is allowed, use child lineage for isolated work that is stacked under or dependent on the active worktree, and use `--no-parent` when it is not stacked. Decide the Git base separately: `--no-parent` makes the worktree top-level in Veer, while omitted `--base-branch` uses the repo default base.
...
Use `veer worktree create --prompt ...` or `veer terminal send ...` for full handoffs or untracked/lightweight prompts. Those paths do not attach `taskId`/`dispatchId`; the worker should not send lifecycle messages unless the prompt supplies a live orchestration preamble.
...
```bash
veer terminal list [--worktree <selector>] [--include-visual-layouts] [--json]
veer terminal create [--worktree <selector>] [--title <text>] [--command <cmd>] [--json]
veer terminal split --terminal <handle> [--direction horizontal|vertical] [--command <cmd>] [--json]
veer terminal wait --terminal <handle> --for tui-idle --timeout-ms <n> --json
veer terminal read --terminal <handle> --json
veer terminal send --terminal <handle> --text <text> --enter --json
```

If an older CLI rejects `worktree create --agent`, create the worktree normally, then run `veer terminal create --worktree <selector> --command "codex" --json` or `--command "claude"`.
...
- Workers with a valid live preamble must send `worker_done` exactly once from their own terminal with an explicit `--outcome succeeded` or `--outcome failed`:
  `veer orchestration send --type worker_done --subject "<short status>" --body "<3-sentence summary: what you did, what you found, what's left>" --task-id <task_id> --dispatch-id <dispatch_id> --outcome succeeded --files-modified "path/a" --report-path "<optional>" --json`
- A failed outcome is still a terminal report, but Veer records both the Dispatch and Task as failed. Never encode failure only in the subject/body.
...
- For long tasks, send heartbeat/status only when the preamble asks for it, including both IDs:
  `veer orchestration send --type heartbeat --subject "alive" --payload '{"taskId":"<task_id>","dispatchId":"<dispatch_id>","phase":"implementing"}' --json`
...
```bash
veer terminal create --worktree active --title login-css-worker --command "claude" --json
veer terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
veer orchestration task-create --spec "Fix the login button CSS" --json
veer orchestration dispatch --task <task_id> --to <handle> --inject --json
veer orchestration check --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```
